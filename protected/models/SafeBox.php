...
class SafeBox extends CFormModel
{
	...
	public function fileWorks()
	{
		...
		$criteria=array('mailHash'=>$this->username);
		$userId="";

		if ($password = Yii::app()->mongo->findOne('user',$criteria,array('password'=>1,'_id'=>1))) {

			if($password['password']==crypt(strtolower($this->password),$password['password']))
			{
				$userId=(string) $password['_id'];
				if($userPlanString=Yii::app()->mongo->findById('user',(string) $password['_id'],array('planData'=>1))) {
		...
		if($correct){
			...
			if($this->action=='PUT'){
				if($userObj=Yii::app()->db->createCommand("SELECT fileObjects FROM safeBoxStorage WHERE userId=:id")->queryRow(true,array(':id'=>$userId))){
					...
					if(isset($decodedHexObject[hash('sha512',$fname)])){
					...
					}else if(count($decodedHexObject)<
						5
						//$userPlanArray['filePerSafeBox']
					){
						...
						header($_SERVER['SERVER_PROTOCOL'] . ' 406 Maximum File limit of ', true, 406);
